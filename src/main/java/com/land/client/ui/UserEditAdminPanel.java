...
public class UserEditAdminPanel extends Composite {
    ...
    private StyledFlowPanel panel = new StyledFlowPanel("ГыукEditPanel form-horizontal", loginPanel, namePanel, passwordChangePanel, passwordNewPanel);

    private UserDto object = null;

    public UserEditAdminPanel() {
        super();
        initWidget(panel);
    ...
    private void setObject(UserDto user) {
        ...
        loginInput.setEnabled(user.getId() == null);
        passwordChangePanel.setVisible(user.getId() != null);
        passwordChangeInput.setValue(user.getId() == null);
        passwordNewPanel.setVisible(user.getId() == null);
    ...
    public static void showInPopup(UserDto user, final IPopupHandler<UserDto> callback) {
        final UserEditAdminPanel editPanel = new UserEditAdminPanel();
    ...
    public void setLoginHelpText(String text) {
        if (!text.isEmpty()) {
            loginPanel.addStyleName("error");
            loginHelp.setText(text);
        }
        else {
            loginPanel.removeStyleName("error");
            loginHelp.setText("");
